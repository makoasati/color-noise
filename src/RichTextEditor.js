...
function ImageModal({ onInsert, onClose }) {
  ...
  const handleInsert = () => {
    if (!canInsert) return;
    onInsert(tab === 'upload' ? base64Data : url.trim());
  };
            ...
            {[['upload', 'Upload File'], ['url', 'Image URL']].map(([t, label]) => (
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  color: tab === t ? '#E73B2F' : '#8A8A8A',
                  borderBottom: tab === t ? '2px solid #E73B2F' : '2px solid transparent',
                  marginBottom: -1,
                }}
              >
                {label}
        ...
        <div style={{ padding: 24 }}>
          {tab === 'upload' ? (
            <>
              ...
              {previewSrc && (
                <img
                  src={previewSrc}
                  alt="Preview"
                  style={{ width: '100%', maxHeight: 200, objectFit: 'contain', display: 'block', border: '1px solid #CCC5B8', marginBottom: 16 }}
                />
              )}
            </>
          ) : (
            <>
              ...
              <input
                ...
                placeholder="https://..."
                style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, background: '#F5F1E8', border: '1px solid #CCC5B8', color: '#111', padding: '10px 12px', width: '100%', boxSizing: 'border-box', marginBottom: 12 }}
              />
              {url && (
                <img
                  src={url}
                  alt="Preview"
                  style={{ width: '100%', maxHeight: 200, objectFit: 'contain', display: 'block', border: '1px solid #CCC5B8' }}
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 20 }}>
            <button
              type="button"
              onClick={onClose}
              style={{ fontFamily: "'Archivo Narrow', sans-serif", fontSize: 11, textTransform: 'uppercase', letterSpacing: '2px', padding: '10px 20px', cursor: 'pointer', background: 'transparent', color: '#8A8A8A', border: '1px solid #CCC5B8', fontWeight: 500 }}
            >
              Cancel
            </button>
            <button
              ...
              disabled={!canInsert}
              style={{ fontFamily: "'Archivo Narrow', sans-serif", fontSize: 11, textTransform: 'uppercase', letterSpacing: '2px', padding: '10px 20px', cursor: canInsert ? 'pointer' : 'default', background: '#E73B2F', color: '#fff', border: 'none', fontWeight: 700, opacity: canInsert ? 1 : 0.35 }}
            >
              Insert
            </button>
          </div>
    ...
    >
      <div style={{ background: '#fff', width: 400, maxWidth: '92vw', padding: 28, border: '1px solid #CCC5B8' }}>
        ...
        <input
          ...
          placeholder="https://..."
          style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, background: '#F5F1E8', border: '1px solid #CCC5B8', color: '#111', padding: '10px 12px', width: '100%', boxSizing: 'border-box' }}
        />
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          {isActive && (
            <button
              type="button"
              onClick={remove}
              style={{ fontFamily: "'Archivo Narrow', sans-serif", fontSize: 11, textTransform: 'uppercase', letterSpacing: '2px', padding: '10px 16px', cursor: 'pointer', background: 'transparent', color: '#E73B2F', border: '1px solid #E73B2F', fontWeight: 500 }}
            >
              Remove
            </button>
          )}
          ...
          <button
            type="button"
            onClick={onClose}
            style={{ fontFamily: "'Archivo Narrow', sans-serif", fontSize: 11, textTransform: 'uppercase', letterSpacing: '2px', padding: '10px 16px', cursor: 'pointer', background: 'transparent', color: '#8A8A8A', border: '1px solid #CCC5B8', fontWeight: 500 }}
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={apply}
            style={{ fontFamily: "'Archivo Narrow', sans-serif", fontSize: 11, textTransform: 'uppercase', letterSpacing: '2px', padding: '10px 16px', cursor: 'pointer', background: '#E73B2F', color: '#fff', border: 'none', fontWeight: 700 }}
          >
            Apply
          </button>
        </div>
  ...
  return (
    <div style={{ border: '1px solid #CCC5B8', background: '#fff' }}>
      ...
      <div style={{
        ...
        <ToolbarBtn active={editor.isActive('heading', { level: 4 })} onClick={() => editor.chain().focus().toggleHeading({ level: 4 }).run()} title="Heading 4">
          H4
        </ToolbarBtn>
        ...
        {/* Insert */}
        <ToolbarBtn active={editor.isActive('link')} onClick={() => setLinkModalOpen(true)} title="Link">
          ↗
        </ToolbarBtn>
        <ToolbarBtn active={false} onClick={() => setImageModalOpen(true)} title="Insert image">
          ⊞
        </ToolbarBtn>
      ...
      {/* Modals */}
      {imageModalOpen && (
        <ImageModal onInsert={handleInsertImage} onClose={() => setImageModalOpen(false)} />
      )}
      {linkModalOpen && (
        <LinkModal editor={editor} onClose={() => setLinkModalOpen(false)} />
      )}
    </div>
  );
}
